...
//		cyl2cart(0., 0., 0.5, points[1][0], points[1][1], points[1][2]);
//		cyl2cart(1., phi2, 0.5, points[2][0], points[2][1], points[2][2]);
//		cyl2cart(1., phi1, 0.5, points[3][0], points[3][1], points[3][2]);
...
void make_cylinder_smooth(int sides, double coord)
{
	for (int i = 0; i < sides; ++i) {
		...
		cyl2cart(0., 0., 0., points[0][0], points[0][1], points[0][2], 0, coord);
		cyl2cart(0., 0., 1., points[1][0], points[1][1], points[1][2], 0, coord);
		cyl2cart(0.2, phi2, 1., points[2][0], points[2][1], points[2][2], 0, coord);
		cyl2cart(0.2, phi1, 1., points[3][0], points[3][1], points[3][2], 0, coord);
		cyl2cart(0.2, phi1, 0., points[4][0], points[4][1], points[4][2], 0, coord);
		cyl2cart(0.2, phi2, 0., points[5][0], points[5][1], points[5][2], 0, coord);

		cyl2cart(0., 0., 1., normals[0][0], normals[0][1], normals[0][2], 0, coord);
		cyl2cart(0., 0., -1., normals[1][0], normals[1][1], normals[1][2], 0, coord);
		cyl2cart(1., phi1, 0., normals[2][0], normals[2][1], normals[2][2], 0, coord);
		cyl2cart(1., phi2, 0., normals[3][0], normals[3][1], normals[3][2], 0, coord);
...
void make_cone_smooth(int sides, double cdx, double cdy, double cdzUP, double cdzDOWN, double cdW)
{
	...
	for (int side = 0; side < sides; side++){
		...
		double x1, x2, y1, y2, z1, z2, xn, yn, zn;
		double xn1, xn2, yn1, yn2, zn1, zn2;
		cyl2cart(cdW, phi1, 0., x1, y1, z1, 0, cdy);
		cyl2cart(cdW, phi2, 1., x2, y2, z2, 0, cdy);
		sph2cart(1., phin, Pi/4., xn, yn, zn, 0, 0, 0);
		sph2cart(1., phi1, Pi/4., xn1, yn1, zn1, 0, 0, 0);
		sph2cart(1., phi2, Pi/4., xn2, yn2, zn2, 0, 0, 0);
		glBegin(GL_TRIANGLES);
...
void make_sphere_flat(int cols, int rows)
{
}
